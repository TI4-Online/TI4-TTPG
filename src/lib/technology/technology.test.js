...
describe("drawTechnologyCard", () => {
    const x89BactrerialWeaponOmegaCard = new MockCard({
        allCardDetails: [
            new MockCardDetails({
                metadata:
                    "card.technology.green:base/x89_bacterial_weapon.omega",
    ...
    const x89BactrerialWeaponCard = new MockCard({
        allCardDetails: [
            new MockCardDetails({
                metadata: "card.technology.green:base/x89_bacterial_weapon",
            }),
        ],
    });
    const darkEnergyTabCard = new MockCard({
        allCardDetails: [
            new MockCardDetails({
                metadata: "card.technology.blue:pok/dark_energy_tap",
            }),
    ...
    const _world_getAllObjects = world.getAllObjects;
    const _PlayerDesk_getClosest = PlayerDesk.getClosest;
    const _card_moveCardToCardHolder = CardUtil.moveCardsToCardHolder;

    const cards = [
        x89BactrerialWeaponCard,
        x89BactrerialWeaponOmegaCard,
        new MockCard({
            allCardDetails: [
                new MockCardDetails({
                    metadata:
        ...
        }), // dark energy tab form another player
        darkEnergyTabCard,
        new MockCard({
            allCardDetails: [
                new MockCardDetails({
                    metadata: "card.technology.blue:basic/fleet_logistics",
                }),
            ],
        }),
    ];
    ...
    afterEach(() => {
        ...
        world.getAllObjects = _world_getAllObjects;
        PlayerDesk.getClosest = _PlayerDesk_getClosest;
        CardUtil.moveCardsToCardHolder = _card_moveCardToCardHolder;

        world.__clear();
        jest.resetAllMocks();
    });

    it("without a technologyName", () => {
        expect(Technology.getOwnedPlayerTechnologies).toThrowError();
    });

    it("without a player slot", () => {
        expect(
            Technology.getOwnedPlayerTechnologies.bind(
                undefined,
    ...
    it("with an unknown technology card", () => {
        expect(
            Technology.getOwnedPlayerTechnologies.bind(
                undefined,
                "algebra",
    ...
    it("with a playerSlot and an omega card available", () => {
        jest.spyOn(world, "getAllObjects").mockReturnValue(cards);

        const moveCardsToCardHolderMock = jest
            .spyOn(CardUtil, "moveCardsToCardHolder")
            .mockReturnValue();

        const playerDeskGetClosestMock = jest
            .spyOn(PlayerDesk, "getClosest")
            .mockReturnValueOnce({ playerSlot: 1 })
            .mockReturnValueOnce({ playerSlot: 1 })
            .mockReturnValueOnce({ playerSlot: 2 })
            .mockReturnValueOnce({ playerSlot: 2 })
        ...
        Technology.drawTechnologyCard("x89_bacterial_weapon", playerSlot);

        expect(moveCardsToCardHolderMock.mock.calls[0][0]).toBe(
            x89BactrerialWeaponOmegaCard
        );
        expect(moveCardsToCardHolderMock.mock.calls[0][1]).toBe(playerSlot);

        // break the loop after finding an omega card
        expect(playerDeskGetClosestMock.mock.calls.length).toBe(2);
    });

    it("with a playerSlot and an omega card UNavailable", () => {
        jest.spyOn(world, "getAllObjects").mockReturnValue(cards);
        ...
        Technology.drawTechnologyCard("dark_energy_tap", playerSlot);

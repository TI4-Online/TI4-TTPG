...
const SYSTEM_SCHEMA = {
    ...
    properties: {
        ...
        wormholes: {type: "array", items: {enum: ["alpha", "beta", "gamma", "delta"]}},
        anomalies: {type: "array", items: {enum: ["asteroid field", "supernova", "nebula", "gravity rift"]}},
        offMap: {type: "boolean"},
...
class SystemSchema {
    ...
    static validate(system, onError) {
        ...
        if (!_systemValidator) {
            _systemValidator = new Ajv({useDefaults: true})
                .compile(SYSTEM_SCHEMA)
        }

...
class Polygon {
    constructor(points) {
        ...
        points.forEach(p => { assert(typeof p.x == 'number' && typeof p.y == 'number') })
    ...
    drawDebug() {
        ...
        for (let i = 0; i < this._polygon.length; i++) {
            world.drawDebugLine(p[i], p[(i + 1) % p.length], color, duration, thickness)
        }
    }
    ...
    contains(point) {
        ...
        if (point.x < this._boundingBox.left || 
        ...
        for (let i = 0, j = this._polygon.length - 1; i < this._polygon.length; i++) {
            ...
            if (((this._polygon[i].y > point.y) !== (this._polygon[j].y > point.y))
                // ...and the edge does not cross our Y corrdinate before our x coordinate (but between our x coordinate and infinity)
                && (point.x < ((this._polygon[j].x - this._polygon[i].x) * (point.y - this._polygon[i].y) / (this._polygon[j].y - this._polygon[i].y) + this._polygon[i].x))) {
                // Invert odd
                odd = !odd
            }
            j = i;
    ...
    inset(amount) {
        ...
        let insetCorner = function(prev, cur, next) {
            ...
            let inset1 = { x : d1.y * amount / dist1, y : -d1.x * amount / dist1 }
            let prev1 = { x : prev.x + inset1.x, y : prev.y + inset1.y }
            let prev2 = { x : cur.x + inset1.x, y : cur.y + inset1.y }
            let inset2 = { x : d2.y * amount / dist2, y : -d2.x * amount / dist2 }

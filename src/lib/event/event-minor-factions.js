...
class EventMinorFactions {
    ...
    static getUnusedSystemTileNumber(redOrBlue) {
        const activeSystemTileNumbers =
            EventMinorFactions.getActiveSystemTileNumbers();
    ...
    constructor(gameObject) {
        gameObject.setDescription("Right click to spawn system");
    ...
    _spawnTile(gameObject, player) {
        ...
        const tileNumber =
            EventMinorFactions.getUnusedSystemTileNumber(redOrBlue);
        ...
        Broadcast.chatAll(
            `Minor Factions Event: ${name} rolled ${roll} to drew a ${redOrBlue} tile`
        );
        Broadcast.chatAll(`${name} drew tile ${tileNumber}: ${summary}`);

        const systemPos = gameObject.getPosition().add([0, 0, 3]);

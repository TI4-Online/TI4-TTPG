...
function getActionCardDeckAndDiscard() {
    for (const obj of world.getAllObjects()) {
        ...
        if (nsid === "mat:base/decks") {
            ...
            if (!deck && !discard) {
                Broadcast.chatAll("Missing both action card deck and pile!");
                return;
            }
 ...
 * @returns {number}
 */
function getNumberOfActionCards(playerSlot) {
 ...
 * @returns {number}
 */
function getNumberOfCommandTokens(playerSlot) {
...
function dealActionCards() {
    const actionCards = getActionCardDeckAndDiscard();
    if (!actionCards) {
    ...
    for (const playerSlot of FindTurnOrder.order()) {
        ...
        const howManyToDeal = getNumberOfActionCards(slot);
        ...
        if (actionCards.deck.getStackSize() > howManyToDeal) {
            Broadcast.chatAll(message);
            actionCards.deck.deal(howManyToDeal, [slot], false, true);
        } else {
 ...
 */
function getCommandTokenBag(playerSlot) {
    for (const obj of world.getAllObjects()) {
        if (obj.getContainer()) {
...
function returnCommandTokens() {
    ...
    const commandTokenBags = Object.fromEntries(
        world.TI4.getAllPlayerDesks().map((element) => [
            element.playerSlot,
            getCommandTokenBag(element.playerSlot),
        ])
    );
 ...
 */
function distributeCommandTokens() {
    for (const playerDesk of world.TI4.getAllPlayerDesks()) {
        const playerSlot = playerDesk.playerSlot;
        const commandTokenBag = getCommandTokenBag(playerSlot);
        const commandTokensToReceive = getNumberOfCommandTokens(playerSlot);

        const message = locale("ui.message.distribute_command_tokens", {
            playerColor: playerDesk.colorName,
            count: commandTokensToReceive,
        });
        const errorMessage = locale("ui.message.not_enough_command_tokens", {
            playerColor: playerDesk.colorName,
        });
        Broadcast.chatAll(message);

        for (let i = 0; i < commandTokensToReceive; i++) {
...
/**
 * Repairs ships, return strategy cards, return command tokens, refresh all
 * exhausted cards, and distribute new command tokens
 */

...
it("when a leadership play button is clicked", () => {
    ...
    jest.spyOn( global.world, "createObjectFromTemplate").mockReturnValue(gameObject);
...
describe("when a player has done the strategy selection", () => {
    ...
    expect(player1Spy).toBeCalledWith("one gained 0 command tokens.", player1.getPlayerColor());
    expect(player2Spy).toBeCalledWith("one gained 0 command tokens.", player1.getPlayerColor());
});
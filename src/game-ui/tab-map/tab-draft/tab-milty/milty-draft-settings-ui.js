...
class MiltyDraftSettingsUI extends VerticalBox {
    ...
    _createDraftSettingsUI(persistentMemory) {
        ...
        onFinishedButton.onClicked.add((button, player) => {
            ...
            const customInputValue = customInput.getText().replaceAll("\\n", " ");
    ...
    _createCustomSelectionUI(persistentMemory) {
        ...
        const factionGridH1 = new HorizontalBox()
            .setChildDistance(CONFIG.spacing * 0.75);
        const factionGridH2 = new HorizontalBox()
            .setChildDistance(CONFIG.spacing * 0.75);
        const factionGridH3 = new HorizontalBox()
            .setChildDistance(CONFIG.spacing * 0.75);
        ...
        world.TI4.getAllFactions().forEach((faction) => {
            const keleresVariants = ["keleres_argent", "keleres_xxcha", "keleres_mentak"];
            const variantColor = [ColorUtil.colorFromHex("#ce3f17"), ColorUtil.colorFromHex("#279D73"), ColorUtil.colorFromHex("#482706")];
            ...
            if (keleresVariants.includes(faction.nsidName)) {
                image = faction.icon.split("keleres")[0] + faction.nsidName + "_icon.png"
            }
            ...
            factionButton.onClicked.add((button, player) => {
                const newMemory = this._callbacks.onFaction(faction.nsidName, sliceInput.getText(), player)
                this._createCustomSelectionUI(newMemory);
            });
            const factionBorder = new Border()
                .setColor(color);
            if (keleresVariants.includes(faction.nsidName)) {
                const keleresBorder = new Border()
                    .setChild(factionButton)
                    .setColor(variantColor[keleresVariants.indexOf(faction.nsidName)])
        ...
        onFinishedButton.onClicked.add((button, player) => {
            ...
            const customInputValue = sliceInput.getText().replaceAll("\\n", " ") + factions;
            const success = this._callbacks.onFinish(customInputValue, player);
            if (success) {
                this._createDraftInProgressUI(persistentMemory);
            }

...
class AutoRollerUI extends Border {
    ...
    resetAwaitingSystemActivation() {
        ...
        const addLayoutButton = (parent, localeText, combatType, planet, fontSize, maxWidth, maxHeight) => {
            assert(typeof parent === "object")
            const lbox1 = new LayoutBox().setOverrideWidth(maxWidth).setOverrideHeight(maxHeight);
            const button = new Button().setText(locale(localeText)).setFontSize(fontSize);
    ...
    resetAfterSystemActivation(system) {
        ...
        const addButton = (parent, localeText, combatType, planet) => {
            assert(typeof parent === "object")
            const button = new Button().setText(locale(localeText)).setFontSize(9);
            button.onClicked.add((button, player) => {
                this._onButton(combatType, planet, player);
            });
        ...
        const addLayoutText = (parent, localeText, fontSize, italize, maxWidth, maxHeight, fontfile = "") => {
            assert(typeof parent === typeof [])
            const text = new Text().setText(locale(localeText)).setJustification(TextJustification.Center).setFontSize(fontSize).setItalic(italize).setFont(fontfile, refPackageId);
            const lbox1 = new LayoutBox().setOverrideWidth(maxWidth).setOverrideHeight(maxHeight);
            const lbox2 = new LayoutBox().setVerticalAlignment(2).setHorizontalAlignment(2);
            const hpanel = new HorizontalBox().setChildDistance(5);
            lbox1.setChild(text);
        ...
        const addHorizontalSubPanel = (parent, spacing) => {
            ...
            const lbox = new LayoutBox().setVerticalAlignment(2).setHorizontalAlignment(2);
            lbox.setChild(panel);
        ...
        const addPlanetSteps = (parent, planet) => {
            ...
            addButton(parent, locale("ui.roller.bombardment"), "bombardment", planet);
            addButton(parent, locale("ui.roller.space_cannon_defense"), "spaceCannonDefense", planet);
            addButton(parent, locale("ui.roller.ground_combat"), "groundCombat", planet);
            parent.pop();
        };

...
class UnitBags {
    ...
    layoutArc(centerPosition, arcOrigin, distanceBetweenBags) {
        ...
        for (let i = 0; i < numBags; i++) {
            const unitBag = this._unitBags[i]
            unitBag.pos = local0.rotateAngleAxis(dYaw * i, [0, 0, 1]).add(arcOrigin)
    ...
    setup(playerSlot) {
        ...
        for (const unitBag of this._unitBags) {
            ...
            unitBag.obj = world.createObjectFromTemplate(unitBag.bagTemplateId, unitBag.pos)
            ...
            for (let i = 0; i < unitBag.unitCount; i++) {
                ...
                const unitObj = world.createObjectFromTemplate(unitBag.unitTemplateId, pos)

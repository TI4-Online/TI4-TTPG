...
fs.ensureDir(`./assets/Templates/tokens/ti4/faction`, 0o2775).then(() => {
    return Promise.all([
        ...Object.entries(FACTIONS).map(([slug, { name, cmdGuid }]) => {
            const guid = cmdGuid ? cmdGuid : generateUuid();
            fs.writeJson(`./assets/Templates/tokens/ti4/faction/${guid}.json`, generateCommand(guid, "ti4", name, slug))
        }),
        Object.entries(FACTIONS).map(([slug, { name, ownGuid }]) => {
            const guid = ownGuid ? ownGuid : generateUuid();
            fs.writeJson(`./assets/Templates/tokens/ti4/faction/${guid}.json`, generateOwner(guid, "ti4", name, slug))
        })
    ])
}).then(() => {
    console.log("done");

...
it("CardData en face with shared back", async () => {
    ...
    assert.equal(
        cardData.face(),
        path.normalize("prebuild/Textures/en/card/action/ghost_ship.jpg")
    );
    assert(!cardData.isFaceGlobal());
    assert.equal(
        cardData.back(),
        path.normalize("prebuild/Textures/global/card/action.back.jpg")
    );
...
it("AssetFilenames.cardImage en face with custom back", () => {
    ...
    assert.equal(
        face,
        path.normalize("prebuild/Textures/en/card/planet/lodor.face.jpg")
    );
    assert.equal(
        back,
        path.normalize("prebuild/Textures/en/card/planet/lodor.back.jpg")
    );
});
...
it("AssetFilenames.cardsheetImage en face", () => {
    ...
    assert.equal(
        face,
        path.normalize("assets/Textures/en/card/action/base/0.face.jpg")
    );
});
...
it("AssetFilenames.cardsheetImage shared back", () => {
    ...
    assert.equal(
        face,
        path.normalize("assets/Textures/en/card/action.back.jpg")
    );
});
...
it("AssetFilenames.templateJson", () => {
    ...
    assert.equal(
        json,
        path.normalize("assets/Templates/card/action/base/0.json")
    );
});
...
it("CardsheetLayout.groupBySource", () => {
    ...
    const cardDataArray = [new CardData(base, "en"), new CardData(pok, "en")];
...
it("CardsheetLayout.groupByBackStyle", () => {
    ...
    const sizeStrToCardDataArrays =
        CardsheetLayout.groupByBackStyle(cardDataArray);
...
it("CardsheetLayout.groupBySize", async () => {
    ...
    const cardDataArray = [new CardData(tall, "en"), new CardData(wide, "en")];
